        ...
        <div class="mt-24 lg:w-1/3">
            <div class="lg:mx-4 xl:mx-8 max-w-sm flex flex-col h-full">
              <div class="bg-cover bg-center h-80 lg:h-64 rounded rounded-b-none">
                <img src="/storage/cover_images/{{$post->image}}" class="object-cover h-full w-full rounded" alt="">
              </div>
              <div class="p-6 rounded border-2 border-t-0 rounded-t-none border-dashed border-indigo-900 flex-1 flex flex-col items-center text-center lg:block lg:text-left">
                ...
                <p class="mt-2 text-sm text-secondary-100 text-justify">
                  {{$post->body}}
                </p>
                <div class="flex justify-center">
                  <a href="{{route('landing.show', ['postName' => strtr($post->title, " ", "-")])}}" class="inline-block px-8 py-2 mt-2 font-bold rounded bg-indigo-500 text-gray-100 hocus:bg-indigo-700 hocus:text-gray-200 focus:shadow-outline focus:outline-none transition duration-300">{{__('Read post')}}</a>
                </div>
              </div>
            </div>
          </div>
